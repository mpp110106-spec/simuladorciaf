...
BEGIN
  ...
  IF v_asesor_id IS NOT NULL THEN
    SELECT a.nombre, a.tiempo_promedio_min
    INTO v_asesor_nombre, v_tprom
    FROM public.asesores AS a
    WHERE a.id = v_asesor_id;
  ...
  SELECT
    v_id AS id,
    v_numero AS numero,
    v_asesor_id AS asesor_id,
    v_asesor_nombre AS asesor_nombre,
    v_personas AS personas_delante,
    (COALESCE(v_personas, 0) * COALESCE(v_tprom, 15)) AS tiempo_estimado_min;
END;
$$;
